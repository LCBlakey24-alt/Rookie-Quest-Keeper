...
async def admin_mission_overview(username: str = Depends(get_current_user)):
    ...
    active_feedback_count = await db.improvement_feedback.count_documents({'status': {'$in': ['reviewing', 'planned', 'in_progress']}})
    done_feedback_count = await db.improvement_feedback.count_documents({'status': 'done'})
    ...
    recent_audit_count = await db.admin_audit_log.count_documents({})
