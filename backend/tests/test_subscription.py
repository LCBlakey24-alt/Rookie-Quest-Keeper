...
BASE_URL = os.environ.get('REACT_APP_BACKEND_URL', 'https://rook-gm-tools.preview.emergentagent.com').rstrip('/')
...
class TestSubscriptionCheckout:
    ...
    def test_create_checkout_session_for_paid_plans(self, auth_user):
        ...
        response = requests.post(
            ...
            headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json"},
            json={"plan_id": "player", "billing_cycle": "monthly", "origin_url": "https://rook-gm-tools.preview.emergentagent.com"}
        )
    ...
    def test_checkout_cannot_checkout_free_plan(self, auth_user):
        ...
        response = requests.post(
            f"{BASE_URL}/api/subscription/checkout",
            headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json"},
            json={"plan_id": "free", "billing_cycle": "monthly", "origin_url": "https://rook-gm-tools.preview.emergentagent.com"}
        )
    ...
    def test_checkout_monthly_billing_cycle(self, auth_user):
        ...
        response = requests.post(
            f"{BASE_URL}/api/subscription/checkout",
            headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json"},
            json={"plan_id": "legendary", "billing_cycle": "monthly", "origin_url": "https://rook-gm-tools.preview.emergentagent.com"}
        )
    ...
    def test_checkout_yearly_billing_cycle(self, auth_user):
        ...
        response = requests.post(
            f"{BASE_URL}/api/subscription/checkout",
            headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json"},
            json={"plan_id": "gm", "billing_cycle": "yearly", "origin_url": "https://rook-gm-tools.preview.emergentagent.com"}
        )
