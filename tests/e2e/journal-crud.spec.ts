...
test.describe('Session Journal CRUD', () => {
  ...
  test('Create journal entry - full flow', async ({ page, request }) => {
    ...
    const response = await request.get('https://rook-gm-tools.preview.emergentagent.com/api/player/journal', {
      headers: { Authorization: `Bearer ${token}` }
    });
    const entries = await response.json();
    const entry = entries.find((e: any) => e.title === entryTitle);
    if (entry) {
      await request.delete(`https://rook-gm-tools.preview.emergentagent.com/api/player/journal/${entry.id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
    }
  });
...
test.describe('Session Journal API Tests', () => {
  test('Create and delete journal entry via API', async ({ page, request }) => {
    ...
    // Create entry
    const createRes = await request.post('https://rook-gm-tools.preview.emergentagent.com/api/player/journal', {
      headers: { Authorization: `Bearer ${token}` },
      data: {
        title: entryTitle,
    ...
    // Delete entry
    const deleteRes = await request.delete(`https://rook-gm-tools.preview.emergentagent.com/api/player/journal/${entry.id}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
