...
test.describe('API-based GM Workflow', () => {
  test('Create NPC via API', async ({ request }) => {
    ...
    // Get auth token
    const loginRes = await request.post('https://midnight-neon-gm.preview.emergentagent.com/api/auth/login', {
      data: {
    ...
    // Create NPC via API
    const createRes = await request.post(`https://midnight-neon-gm.preview.emergentagent.com/api/campaigns/${TEST_CAMPAIGN_ID}/npcs`, {
    ...
    // Cleanup - Delete NPC
    const deleteRes = await request.delete(`https://midnight-neon-gm.preview.emergentagent.com/api/campaigns/${TEST_CAMPAIGN_ID}/npcs/${npc.id}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
  ...
  test('Create Location via API', async ({ request }) => {
    ...
    const createRes = await request.post(`https://midnight-neon-gm.preview.emergentagent.com/api/campaigns/${TEST_CAMPAIGN_ID}/locations`, {
      headers: { Authorization: `Bearer ${token}` },
      data: {
        name: locationName,
        location_type: 'City',
    ...
    const deleteRes = await request.delete(`https://midnight-neon-gm.preview.emergentagent.com/api/campaigns/${TEST_CAMPAIGN_ID}/locations/${location.id}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    expect(deleteRes.ok()).toBeTruthy();
  });
  ...
  test('Create Campaign via API', async ({ request }) => {
    ...
    const createRes = await request.post(`https://midnight-neon-gm.preview.emergentagent.com/api/campaigns`, {
      headers: { Authorization: `Bearer ${token}` },
      data: {
        name: campaignName,
    ...
    const deleteRes = await request.delete(`https://midnight-neon-gm.preview.emergentagent.com/api/campaigns/${campaign.id}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    expect(deleteRes.ok()).toBeTruthy();
  });
