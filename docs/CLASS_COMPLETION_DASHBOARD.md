...
| 1 | Wizard | 70% | Next | Build Wizard sheet summary, final status, package export, and tests. |
...
Continue Wizard next. Progression, character detection, school summaries, and builder readiness are now started, so the next useful step is the Wizard sheet summary.
