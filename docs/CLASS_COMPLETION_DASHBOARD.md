...
| 1 | Warlock | 20% | Next | Build Warlock progression, patron summaries, builder readiness, sheet summary, final status, package export, and tests. |
...
Start Warlock next. Wizard is now complete across the checklist, so Warlock becomes the next class package to build using the same small-PR pattern.
