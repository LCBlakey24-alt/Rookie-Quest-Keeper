...
| 1 | Wizard | 80% | Next | Build Wizard final status, package export, and tests. |
...
Continue Wizard next. Progression, character detection, school summaries, builder readiness, and sheet summary are now started, so the next useful step is final status plus the package export layer.
