...
export function getMonkBuilderChoiceSummary(level = 1, edition = '2014') {
  ...
  const sections = [{ key: 'subclass', label: 'Subclass', required: options.needsSubclass, count: options.needsSubclass ? 1 : 0, options: options.subclassOptions }].filter(section => section.required);
