...
describe('Barbarian builder options', () => {
  test('returns 2014 and 2024 subclass options', () => {
    expect(getBarbarianSubclassOptions('2014').map(option => option.key)).toEqual(['berserker', 'totem_warrior']);
    expect(getBarbarianSubclassOptions('2024').map(option => option.key)).toEqual(['berserker', 'wild_heart', 'world_tree', 'zealot']);
  });
  ...
  test('validates complete and missing builder selections', () => {
    ...
    expect(validateBarbarianBuilderSelections({ level: 3, edition: '2024', subclass: '', weaponMasteries: [] }).errors).toContain('Choose a Barbarian subclass.');
  });
});
