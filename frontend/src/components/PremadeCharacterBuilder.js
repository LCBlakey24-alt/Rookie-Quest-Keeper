...
const inputStyle = {
  ...
  padding: "11px 12px",
  borderRadius: 8,
  background: rookieTheme.input,
  border: "1px solid rgba(230, 59, 59, 0.34)",
  color: rookieTheme.text,
  fontSize: 14,
  outline: "none",
};
...
function getDifficulty(template) {
  ...
  if (["Fighter", "Barbarian", "Rogue", "Monk"].includes(cls))
    return "Beginner";
...
function getIdentity(template) {
  if (!template) return "";
  const ancestry = template.subrace
    ? `${template.subrace} ${template.race}`
    : template.race;
  return [template.character_class, ancestry, template.background]
    .filter(Boolean)
    .join(" · ");
}
...
function Pill({ children, tone = "neutral" }) {
  return (
    <span className={`premade-pill premade-pill-${tone}`}>{children}</span>
  );
}
...
export default function PremadeCharacterBuilder() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const res = await apiClient.get("/character-templates", {
      ...
      setSelectedTemplate((current) => {
        if (
          current &&
          nextTemplates.some((template) => template.id === current.id)
        )
          return current;
    ...
    load()
      .catch(() => toast.error("Failed to load premade heroes"))
  ...
  const classOptions = useMemo(
    () => [
      "All",
      ...Array.from(
        new Set(templates.map((t) => t.character_class).filter(Boolean)),
      ).sort(),
    ],
    [templates],
  );
  ...
  const filteredTemplates = useMemo(() => {
    ...
    return templates.filter((template) => {
      ...
      const matchesClass =
        classFilter === "All" || template.character_class === classFilter;
      const matchesRole = roleFilter === "all" || tags.includes(roleFilter);
      const matchesDifficulty =
        difficultyFilter === "All" ||
        getDifficulty(template) === difficultyFilter;
  ...
  const selectedIsSuggested =
    match?.best_match?.id && selectedTemplate?.id === match.best_match.id;
  const selectedIsPreparedCaster =
    selectedTemplate &&
    PREPARED_SPELLCASTERS.has(selectedTemplate.character_class);
  ...
  const runMatch = async () => {
    ...
    try {
      ...
      if (suggested) {
        setSelectedTemplate(
          templates.find((template) => template.id === suggested.id) ||
            suggested,
        );
      }
    } catch (e) {
  ...
  const createFromTemplate = async (template) => {
    ...
    try {
      const { data: full } = await apiClient.get(
        `/character-templates/${template.id}`,
      );
      ...
      if (warnings.length) {
        toast.warning(
          `Created with ${warnings.length} sheet detail${warnings.length === 1 ? "" : "s"} to review.`,
        );
      }
      ...
      toast.error(
        e?.response?.data?.detail || "Failed to create character from template",
      );
    } finally {
      ...
      <div className="premade-shell">
        ...
        <section className="premade-hero">
          <div className="premade-hero-copy">
            ...
            <p>
              Premade characters are the fastest route into the game. Choose a
              ready hero, make a copy, then edit the full sheet later when you
              want more control.
            </p>

            <div className="premade-steps" aria-label="Premade character steps">
              <span>
                <strong>1</strong> Choose a style
              </span>
              <span>
                <strong>2</strong> Pick a hero
              </span>
              <span>
                <strong>3</strong> Create the sheet
              </span>
            </div>
          </div>
          ...
          <aside className="premade-help-card">
            ...
            <span>
              These heroes are built to be playable straight away. Rook can
              suggest one, but you can still choose any hero yourself.
            </span>
          </aside>
        </section>

        <section
          className="premade-toolbar"
          aria-label="Premade character setup"
        >
        ...
        <section className="premade-suggest" aria-label="Rook hero suggestion">
          ...
          <button
            onClick={runMatch}
            disabled={loadingMatch || !description.trim()}
          ...
          {match?.best_match ? (
            <div
              className="premade-suggestion"
              data-testid="premade-suggestion"
            >
              <span>Suggested hero</span>
              <strong>
                {match.best_match.name} · {match.best_match.character_class}
              </strong>
            ...
            <div className="premade-suggestion premade-suggestion-empty">
              ...
              <p>
                Describe the vibe you want and Rook will point you toward a
                matching hero.
              </p>
            </div>
          )}
        ...
        <section
          className="premade-filters"
          aria-label="Premade character filters"
        >
          <div className="premade-filter-group">
            ...
            <select
              ...
              data-testid="premade-class-filter"
            >
          ...
          <div className="premade-filter-group">
            ...
            <select
              ...
              data-testid="premade-difficulty-filter"
            >
        ...
        <div className="premade-layout">
          <section className="premade-gallery" aria-label="Premade hero cards">
            <div className="premade-section-title">
              <div>
                <h2>Hero cards</h2>
                <p>
                  Choose a ready-made hero template. You will create your own
                  copy.
                </p>
              </div>
            ...
            {loadingTemplates && (
              <div className="premade-empty">
                Loading ready-to-play templates…
              </div>
            )}

            {!loadingTemplates && templates.length === 0 && (
              <div className="premade-empty">
                No premade heroes are available for this rules edition yet.
              </div>
            )}

            {templates.length > 0 && filteredTemplates.length === 0 && (
              <div className="premade-empty">
                No heroes match those filters yet. Try another role, class,
                difficulty, or search term.
              </div>
            )}
          ...
          <aside
            className="premade-preview"
            data-testid="premade-selected-preview"
          >
            <div className="premade-section-title">
              <div>
                <h2>Selected hero</h2>
            ...
            {selectedTemplate ? (
              <>
                <div className="premade-preview-card">
                  <div className="premade-avatar">
                    {selectedTemplate.name?.slice(0, 1) || "?"}
                  </div>
                  <div>
                ...
                <p className="premade-preview-pitch">
                ...
                <div className="premade-preview-stats">
                  <PreviewStat
                    label="Role"
                    value={getRoleLabel(selectedTemplate)}
                  />
                  <PreviewStat
                    label="Complexity"
                    value={getDifficulty(selectedTemplate)}
                  />
                ...
                {selectedIsPreparedCaster && (
                  <label className="premade-spell-plan" style={labelStyle}>
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        updateSpellPlan(selectedTemplate.id, event.target.value)
                      }
                      style={inputStyle}
                ...
                <div className="premade-tags">
                  {(selectedTemplate.playstyle_tags || [])
                    .slice(0, 8)
                    .map((tag) => (
                ...
                <span className="premade-edit-note">
                  You can edit the full character sheet after creation.
                </span>
              </>
            ) : (
              <div className="premade-empty">
                Choose a hero card to preview it here.
              </div>
            )}
          </aside>
        </div>
...
function HeroCard({
  ...
  return (
    <article
      className={`premade-card ${selected ? "selected" : ""}`}
    ...
    >
      <button type="button" onClick={onSelect} className="premade-card-main">
        ...
        <div className="premade-card-header">
          <div className="premade-avatar small">
        ...
        <div className="premade-tags compact">
          {(template.playstyle_tags || []).slice(0, 4).map((tag) => (
            <span key={tag}>{tag}</span>
          ))}
        </div>
      </button>
...
.premade-page {
  ...
  overflow-y: auto;
  background:
    radial-gradient(circle at top left, rgba(230, 59, 59, 0.14), transparent 32%),
    radial-gradient(circle at top right, rgba(159, 29, 29, 0.18), transparent 34%),
    linear-gradient(180deg, #0D0F12, #15181D);
  color: #F7F7F8;
  padding: 20px;
...
.premade-back {
  ...
  border: 1px solid rgba(230, 59, 59, 0.24);
  color: #A8AFBA;
  cursor: pointer;
  font-size: 12px;
  font-weight: 900;
...
.premade-back:hover {
  border-color: rgba(230, 59, 59, 0.58);
  color: #F7F7F8;
...
border: 1px solid rgba(230, 59, 59, 0.25);
font-size: 12px;
font-weight: 800;
}
...
gap: 7px;
padding: 16px;
background: linear-gradient(135deg, rgba(230, 59, 59, 0.12), rgba(159, 29, 29, 0.15));
...
.premade-primary {
background: linear-gradient(135deg, #E63B3B, #9F1D1D);
border: 1px solid rgba(230, 59, 59, 0.72);
border-radius: 4px;
color: #F7F7F8;
cursor: pointer;
font-weight: 900;
...
display: inline-flex;
color: #F7F7F8;
border: 1px solid rgba(230, 59, 59, 0.46);
background: linear-gradient(135deg, rgba(230, 59, 59, 0.22), rgba(159, 29, 29, 0.24));
border-radius: 4px;
padding: 3px 7px;
font-size: 10px;
font-weight: 900;
...
background: #15181D;
border: 1px solid rgba(230, 59, 59, 0.2);
color: #A8AFBA;
border-radius: 4px;
padding: 7px 10px;
font-size: 11px;
...
border-color: rgba(230, 59, 59, 0.72);
color: #F7F7F8;
background: linear-gradient(135deg, rgba(230, 59, 59, 0.22), rgba(159, 29, 29, 0.24));
}
...
color: #A8AFBA;
padding: 18px;
border: 1px dashed rgba(230, 59, 59, 0.24);
border-radius: 6px;
background: rgba(27, 32, 39, 0.45);
font-size: 13px;
line-height: 1.45;
}
...
border-color: rgba(230, 59, 59, 0.7);
box-shadow: none;
}
...
align-items: center;
justify-content: center;
border: 1px solid rgba(230, 59, 59, 0.36);
background: linear-gradient(135deg, rgba(230, 59, 59, 0.3), rgba(159, 29, 29, 0.32));
color: #F7F7F8;
font-size: 24px;
font-weight: 900;
}
...
.premade-card-create {
border: 0;
border-top: 1px solid rgba(230, 59, 59, 0.18);
background: linear-gradient(135deg, rgba(230, 59, 59, 0.24), rgba(159, 29, 29, 0.26));
color: #F7F7F8;
padding: 10px 12px;
font-size: 11px;
font-weight: 900;
...
border: 0;
border-right: 1px solid rgba(230, 59, 59, 0.18);
...
.premade-preview-card {
background: linear-gradient(135deg, rgba(230, 59, 59, 0.12), rgba(159, 29, 29, 0.14));
border: 1px solid rgba(230, 59, 59, 0.24);
border-radius: 6px;
padding: 12px;
margin-bottom: 11px;
