...
const getFighterManeuverTarget = (fighterLevel) => {
  const level = Number(fighterLevel || 0);
...
export default function LevelUpWizard({ character, isOpen, onClose, onLevelUp }) {
  ...
  const hasExistingFightingStyle = Boolean(character?.fighting_style || character?.fightingStyle);
  const hasFightingStyleChoice = Boolean(
    classData?.fighting_styles
    && (
      classData?.fighting_style_level === newClassLevel
      || (classKey === 'fighter' && !hasExistingFightingStyle && newClassLevel >= (classData?.fighting_style_level || 1))
    )
  );
  ...
  const normalizedSubclass = normalizeName(selectedSubclass || character?.subclass || '');
  const isBattleMaster = normalizedSubclass === 'battlemaster';
  const requiredManeuverCount = isBattleMaster ? getFighterManeuverTarget(newClassLevel) : 0;
  const bmNeedsManeuvers = Boolean(isBattleMaster && requiredManeuverCount > 0 && selectedManeuvers.length < requiredManeuverCount);
  ...
  const canProceed = () => {
    ...
    if (step === classChoiceStep) {
      ...
      if (bmNeedsManeuvers && selectedManeuvers.length < requiredManeuverCount) return false;
      return true;
    }
              ...
              {bmNeedsManeuvers && (
                <div>
                  <p style={{ color: theme.text.secondary, fontSize: '14px', marginBottom: '8px' }}>
                    Choose maneuvers for your Battle Master progression. These special combat techniques are fueled by your superiority dice.
                  </p>
                  <p style={{ color: theme.text.muted, fontSize: '12px', marginBottom: '12px' }}>
                    Selected: {selectedManeuvers.length}/{requiredManeuverCount}
                  </p>
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            ...
                            } else if (selectedManeuvers.length < requiredManeuverCount) {
                          ...
                          style={{
                            ...
                            opacity: !isSelected && selectedManeuvers.length >= requiredManeuverCount ? 0.4 : 1,
                            transition: 'all 0.2s',
                          }}
                        >
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
