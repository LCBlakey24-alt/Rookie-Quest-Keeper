...
const EMPTY_DRAFTS = {
  ...
  custom_rule: { name: '', image_url: '', category: 'other', summary: '', enabled_by_default: true, rule_text: '', trigger: '', resolution: '', examples: [], settings: {}, player_visible: true, gm_notes: '' },
  magic_item: { name: '', image_url: '', type: 'Wondrous Item', rarity: '', requires_attunement: false, description: '', effects: [] },
  race: { name: '', image_url: '', description: '', size: 'Medium', speed: 30, ability_bonuses: {}, traits: [], languages: [] },
  class: { name: '', image_url: '', description: '', hit_die: 'd8', primary_ability: '', saving_throw_proficiencies: [], armor_proficiencies: [], weapon_proficiencies: [], features: [] },
  subclass: { name: '', image_url: '', parent_class: '', description: '', subclass_level: 3, features: [] },
  background: { name: '', image_url: '', description: '', skill_proficiencies: [], tool_proficiencies: [], languages: 0, equipment: [], feature_name: '', feature_description: '' },
};
...
function DraftEditor({ contentType, draft, missing, onChange }) {
  ...
  if (contentType === 'custom_rule') {
    return <div style={formStackStyle}>{art}
      <FormGrid><FieldRow label="Rule Name" value={draft.name} onChange={v => upd('name', v)} missing={miss('name')} placeholder="Exploding d6s, Chaos Tokens, Tech Check..." /><SelectRow label="Category" value={draft.category || 'other'} onChange={v => upd('category', v)} options={CATEGORIES} /></FormGrid>
      ...
      <FormGrid><FieldRow label="Trigger" value={draft.trigger} onChange={v => upd('trigger', v)} placeholder="When does this happen?" /><FieldRow label="Resolution" value={draft.resolution} onChange={v => upd('resolution', v)} placeholder="What does the table do next?" /></FormGrid>
  ...
  return <main style={pageStyle} data-testid="homebrew-workshop">
    <section style={wrapStyle}>
      <header style={headerStyle}>
        ...
        <div style={{ minWidth: 0, flex: 1 }}><p style={eyebrowStyle}>Rook-assisted creation</p><h1 style={titleStyle}><Sparkles size={28} /> Homebrew Workshop</h1><p style={subtitleStyle}>Build monsters, NPCs, items, classes, species, backgrounds, and custom rules. Add optional artwork, fill what you know, then ask Rook to complete the missing pieces.</p></div>
      ...
      <section style={builderGridStyle}>
        <aside style={panelStyle}>
          ...
          <p style={helpStyle}>Upload a file, paste rough notes, or type directly into the draft. Rook can then parse or complete the missing parts. Artwork is added inside the draft panel.</p>
          ...
          <div style={tipStyle}><strong>Good workflow:</strong> fill the name, theme, and two or three key ideas first. Add an image when the homebrew needs a portrait, crest, item card, token, or visual reference.</div>
        </aside>
