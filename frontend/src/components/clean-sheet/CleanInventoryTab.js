...
import { Backpack, Coins, Plus, Search, Shield, Sparkles, Swords } from 'lucide-react';
...
function normaliseItem(item, source = '') {
  if (typeof item === 'string') return { name: item, type: 'Item', item_type: 'Item', quantity: 1, description: source === 'starting' ? 'Starting equipment' : '', source };
...
export default function CleanInventoryTab({ character, onCharacterUpdate, onRoll }) {
  ...
  const starterGear = useMemo(() => toArray(character?.starting_equipment || character?.startingEquipment).map(item => normaliseItem(item, 'starting')), [character]);
  const equipment = useMemo(() => toArray(character?.equipment).map(item => normaliseItem(item, 'equipment')), [character]);
  const inventory = useMemo(() => toArray(character?.inventory).map(item => normaliseItem(item, 'inventory')), [character]);
  const allCarriedItems = useMemo(() => dedupeItems([...equipment, ...starterGear, ...inventory]), [equipment, starterGear, inventory]);
  ...
  const equipItem = (slot, rawItem) => {
    ...
    const equippedItem = { ...item, equip_slot: slot, equipped_slot: slot, equipped: true, is_equipped: true };
  ...
  const updateInventoryItem = async (item, updates) => {
    const index = inventory.findIndex(candidate => getItemName(candidate).toLowerCase() === getItemName(item).toLowerCase());
    if (index < 0) { toast.info('Starter gear can be equipped, but only backpack items can be edited here.'); return; }
  ...
  const removeInventoryItem = async (item) => {
    const index = inventory.findIndex(candidate => getItemName(candidate).toLowerCase() === getItemName(item).toLowerCase());
    if (index < 0) { toast.info('Starter gear can be equipped, but only backpack items can be removed here.'); return; }
    const nextInventory = [...inventory];
  ...
  const makeWeaponRoll = (item) => {
    ...
    toast.success(`${attack.title} attack: ${roll} ${attack.attackText} = ${roll + Number(attack.attackMod || 0)}`, { description: `${attack.damageText} ${attack.damageType || ''} damage if it hits` });
  };
  ...
  const itemActions = (item) => {
    ...
    return (
      <>
        ...
        {(slot === 'mainHand' || item?.damage_dice || item?.damage || String(item?.type || '').toLowerCase().includes('weapon')) && <button type="button" onClick={() => makeWeaponRoll(item)} disabled={savingItems}>Attack</button>}
        {requiresAttunement(item) && isInventoryItem && <button type="button" onClick={() => updateInventoryItem(item, { attuned: !isAttuned(item), is_attuned: !isAttuned(item) })} disabled={savingItems}>{isAttuned(item) ? 'Unattune' : 'Attune'}</button>}
        {isInventoryItem && <button type="button" onClick={() => updateInventoryItem(item, { quantity: Math.max(1, qty - 1), qty: Math.max(1, qty - 1) })} disabled={savingItems}>- Qty</button>}
        {isInventoryItem && <button type="button" onClick={() => updateInventoryItem(item, { quantity: qty + 1, qty: qty + 1 })} disabled={savingItems}>+ Qty</button>}
        {isInventoryItem && <button type="button" onClick={() => updateInventoryItem(item, { favorite: !isFavorite(item), favourite: !isFavorite(item) })} disabled={savingItems}>{isFavorite(item) ? 'Unfav' : 'Fav'}</button>}
        {isInventoryItem && <button type="button" onClick={() => updateInventoryItem(item, { type: isConsumableLike(item) ? 'Item' : 'Consumable' })} disabled={savingItems}>{isConsumableLike(item) ? 'Not Consumable' : 'Consumable'}</button>}
        {isInventoryItem && <button type="button" onClick={() => removeInventoryItem(item)} disabled={savingItems}>Remove</button>}
      </>
    );
  };
    ...
    <div className="clean-sheet-grid clean-sheet-inventory-tab">
      <section className="clean-sheet-panel clean-sheet-wide">
        <div className="clean-sheet-inventory-header">
          <div><h2>Equipped</h2><p>Equipping armour recalculates AC. Equipping weapons adds usable attack rolls.</p></div>
          <button type="button" onClick={() => setShowAddItem(prev => !prev)}><Plus size={15} /> {showAddItem ? 'Close Add Item' : 'Add Item'}</button>
        </div>
        <div className="clean-sheet-item-grid">
          {EQUIP_SLOTS.map(([slot, label]) => {
            const item = getEquippedItem(equipped, slot);
            return item ? <ItemCard key={slot} slot={label} item={normaliseItem(item, 'equipped')} actions={<>{(['mainHand', 'offHand'].includes(slot) || item?.damage_dice || item?.damage || String(item?.type || '').toLowerCase().includes('weapon')) && <button type="button" onClick={() => makeWeaponRoll(item)}>Attack Roll</button>}<button type="button" onClick={() => clearSlot(slot)} disabled={savingSlot === slot}>Clear Slot</button></>} /> : <article key={slot} className="clean-sheet-item-card clean-sheet-empty-slot"><span className="clean-sheet-item-slot">{label}</span><strong>Empty</strong><p>Select an item below to assign this slot.</p></article>;
          })}
        </div>
      </section>
      ...
      {showAddItem && (
        <section className="clean-sheet-panel clean-sheet-wide">
          ...
          <form className="clean-sheet-add-item-form" onSubmit={addItem}>
            ...
            <select value={newItem.damage_type} onChange={e => setNewItem(prev => ({ ...prev, damage_type: e.target.value }))}><option value="">Damage type</option>{['slashing','piercing','bludgeoning','fire','cold','lightning','acid','poison','necrotic','radiant','psychic','thunder','force'].map(t => <option key={t} value={t}>{t}</option>)}</select>
      ...
      <section className="clean-sheet-panel clean-sheet-wide clean-sheet-inventory-summary">
        ...
        <div><Shield size={16} /><span>Equipped</span><strong>{EQUIP_SLOTS.filter(([slot]) => getEquippedItem(equipped, slot)).length}/4</strong></div>
        <div><Sparkles size={16} /><span>Favourites</span><strong>{favoriteItems.length}</strong></div>
      ...
      {favoriteItems.length > 0 && <section className="clean-sheet-panel clean-sheet-wide"><h2>Favourite Items</h2><div className="clean-sheet-item-grid">{favoriteItems.map((item, index) => <ItemCard key={itemKey(item, index)} item={item} actions={itemActions(item)} />)}</div></section>}
      {consumables.length > 0 && <section className="clean-sheet-panel clean-sheet-wide"><h2>Consumables</h2><div className="clean-sheet-item-grid">{consumables.map((item, index) => <ItemCard key={itemKey(item, index)} item={item} actions={itemActions(item)} />)}</div></section>}

      <section className="clean-sheet-panel clean-sheet-wide">
        <div className="clean-sheet-inventory-header"><div><h2>Add Equipment from List</h2><p>Pick a weapon, armour, or shield and add it straight to this character.</p></div></div>
        <div className="clean-sheet-equipment-tools"><label><Search size={15} /><input value={equipmentSearch} onChange={event => setEquipmentSearch(event.target.value)} placeholder="Search weapons, armour, shields…" /></label><select value={equipmentTypeFilter} onChange={event => setEquipmentTypeFilter(event.target.value)} aria-label="Filter equipment type"><option value="all">All equipment</option><option value="weapon">Weapons</option><option value="armor">Armour</option><option value="shield">Shields</option></select></div>
      ...
      <section className="clean-sheet-panel clean-sheet-wide">
        <div className="clean-sheet-inventory-header"><div><h2>Carried Items</h2><p>Includes starter gear, saved equipment, backpack items, and GM-granted items.</p></div><input value={itemSearch} onChange={e => setItemSearch(e.target.value)} placeholder="Search items…" /></div>
        <div className="clean-sheet-item-grid">{filteredInventory.length ? filteredInventory.map((item, index) => <ItemCard key={itemKey(item, index)} item={item} actions={itemActions(item)} />) : <p className="clean-sheet-empty-note">No carried items match your search.</p>}</div>
      </section>
    </div>
  );
}
