...
export default function BarbarianSummarySection({ character, resources = [], onRage }) {
  if (!isBarbarianCharacter(character)) return null;
  const summary = getBarbarianSheetSummary(character);
  const rage = resources.find(resource => resource.key === 'rage');
  ...
  return (
    <section className="clean-sheet-panel clean-sheet-wide clean-sheet-fighter-panel" data-testid="barbarian-summary-panel">
      ...
      <div className="clean-sheet-fighter-buttons">
        <button type="button" onClick={onRage} disabled={!rage || rage.current <= 0}>Rage {rage ? `${rage.current}/${rage.max}` : summary.rageUsesLabel}</button>
      </div>
      {summary.subclassFeatures.length > 0 && <FeatureList title="Subclass online" features={summary.subclassFeatures} />}
      {summary.nextSubclassFeatures.length > 0 && <FeatureList title="Subclass coming next" features={summary.nextSubclassFeatures} />}
