...
describe('RookCharacterMatchmaker local matcher', () => {
  test('returns three sensible Batman-style matches', () => {
    const matches = getRookCharacterMatches({ description: 'I want to be like Batman but in D&D', magic: 'some', complexity: 'balanced' });
  ...
  test('honors a known preferred class when an archetype supports it', () => {
    const matches = getRookCharacterMatches({ description: 'I want music, lies, and chaos', preferredClass: 'Bard' });
