...
describe('SessionTimeline reliability', () => {
  ...
  test('player timeline only shows this campaign and has no GM controls', async () => {
    apiClient.get.mockResolvedValueOnce({ data: [savedEvent, { ...savedEvent, id: 'other', campaign_id: 'campaign-2', title: 'Other campaign' }] });
    render(<SessionTimeline campaignId="campaign-1" readOnly />);
    expect(await screen.findByText(savedEvent.title)).toBeInTheDocument();
    expect(apiClient.get).toHaveBeenCalledWith('/player/timeline');
