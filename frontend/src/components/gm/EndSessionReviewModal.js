...
const metricStyle = (hot, muted) => ({ minHeight: 92, display: 'grid', alignContent: 'center', gap: 5, padding: 12, background: hot ? 'rgba(124,203,255,0.10)' : muted ? rq.bg : rq.card, border: `1px solid ${hot ? rq.pink : rq.line}`, color: rq.text, textAlign: 'center' });
const warningStyle = { background: rq.bg, border: `1px solid ${rq.line}`, borderLeft: `1px solid ${rq.pink}`, padding: 12, color: rq.soft, marginTop: 12, lineHeight: 1.45 };
const storyWrapStyle = { display: 'grid', gap: 12, background: rq.card, border: `1px solid ${rq.line}`, borderLeft: `1px solid ${rq.pink}`, padding: 12, marginTop: 12 };
...
const gmOnlyNoticeStyle = { margin: 0, color: rq.text, background: rq.bg, border: `1px solid ${rq.line}`, borderLeft: `1px solid ${rq.pink}`, padding: '8px 10px', fontSize: 12, fontWeight: 850, lineHeight: 1.4 };
...
const progressBarStyle = (progress) => ({ display: 'block', width: `${Math.max(0, Math.min(100, progress))}%`, height: '100%', background: rq.blue, transition: 'width 240ms ease' });
const storyPreviewGridStyle = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 8 };
const storyCountStyle = (tone) => ({ minHeight: 72, display: 'grid', gap: 4, alignContent: 'center', padding: 10, background: rq.bg, border: `1px solid ${rq.line}`, borderLeft: `1px solid ${tone === 'good' ? rq.blue : rq.pink}`, color: rq.text });
...
const previewRowStyle = { display: 'grid', gap: 3, background: rq.bg, borderLeft: `1px solid ${rq.pink}`, padding: 9, marginTop: 7 };
...
const primaryButtonStyle = { minHeight: 40, border: `1px solid ${rq.pink}`, background: rq.card, color: rq.text, padding: '0 12px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 7, fontWeight: 950, cursor: 'pointer', fontFamily: fontStack };
