...
export default function LiveSessionGridPage() {
  ...
  const syncNoteIntoCampaignState = async (noteId) => {
    ...
    await fetchAllData();
    if (applied.length > 0) toast.success(`Note saved and ${applied.length} campaign update${applied.length === 1 ? '' : 's'} applied`, { description: applied.slice(0, 2).map(update => update.summary).join(' ') });
    else toast.success('Note saved', { description: 'No clear character, NPC, or location changes were detected automatically.' });
  };
