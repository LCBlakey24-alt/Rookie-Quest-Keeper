...
import { AlertTriangle, Check, Loader, MapPin, RefreshCw, Save, Sparkles, Swords, UserRound, X } from 'lucide-react';
...
const TYPES = [
  ...
  { id: 'creature', label: 'Creature', icon: Swords, placeholder: 'A CR 3 plant creature grown from corrupted river roots…' },
];
...
const LONG_FIELDS = new Set(['description', 'appearance', 'personality', 'backstory', 'notes', 'abilities', 'attacks', 'places_of_interest', 'stats', 'saving_throws', 'skills']);
...
export default function RookCreateStudio({ campaignId }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.error('One of the JSON fields is not valid yet', { description: 'Check arrays/objects such as stats, attacks, abilities, or places of interest.' });
      return;
    }
    setSaving(true);
    try {
      ...
      toast.success(`${saved?.name || typeMeta.label} saved to campaign`);
      try {
      ...
      {!draft ? (
        ...
        <div style={draftShellStyle}>
          <header style={draftHeaderStyle}>
            <span><strong>{editorValues.name || `New ${typeMeta.label}`}</strong><small>Rook draft · editable</small></span>
...
const typeGridStyle = { display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 4 };
