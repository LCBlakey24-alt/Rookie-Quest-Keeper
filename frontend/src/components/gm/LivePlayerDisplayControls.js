...
export default function LivePlayerDisplayControls({ campaignId, campaignName = 'Campaign' }) {
  ...
  const sendCombat = () => {
    const participants = selectedScenario?.participants || selectedScenario?.combatants || [];
    publish('combat', {
      title: selectedScenario?.name || sceneTitle || 'Combat',
      map_url: imageUrl.trim() || imageFrom(selectedScenario),
      tokens: participants
        .filter(item => item.type !== 'player' && item.hidden !== true)
        .map(item => ({ id: item.id, name: item.name || item.monster_name || 'Enemy' })),
    });
  };
        ...
        <div style={bodyStyle}>
          <div style={formGridStyle}>
            ...
            <label style={fieldStyle}>
              ...
              <select value={selectedScenarioId} onChange={(event) => setSelectedScenarioId(event.target.value)} style={inputStyle}>
          ...
          <section style={sendRowStyle}>
            ...
            <button type="button" onClick={sendCombat} style={primaryButtonStyle}><Skull size={14} /> Send Combat View</button>
          </section>
...
const miniButtonStyle = { minHeight: 28, border: 0, background: theme.bg, color: theme.text, display: 'inline-flex', gap: 5, alignItems: 'center', padding: '0 8px', fontSize: 11, fontFamily: fontStack };
